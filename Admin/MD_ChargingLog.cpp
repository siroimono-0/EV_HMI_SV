#include "MD_ChargingLog.h"
#include "ui_MD_ChargingLog.h"

MD_ChargingLog::MD_ChargingLog(QObject *parent)
    : QAbstractListModel{parent}
    , ui(new Ui::MD_ChargingLog)
{
    ui->setupUi(this);

    QVector<charging_log_admin> tmp;
    struct charging_log_admin test_data;

    test_data.actual_payment = 10;
    tmp.push_back(test_data);

    this->slot_qvec_update(tmp);
}

MD_ChargingLog::~MD_ChargingLog()
{
    delete ui;
}

int MD_ChargingLog::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
    {
        return 0;
...
QVariant MD_ChargingLog::data(const QModelIndex &index, int role) const
{
...
QHash<int, QByteArray> MD_ChargingLog::roleNames() const
{
...
Q_INVOKABLE void MD_ChargingLog::reset_md()
{
    beginResetModel();
    endResetModel();
}

void MD_ChargingLog::slot_qvec_update(QVector<charging_log_admin> new_qvec)
{
    qDebug() << Q_FUNC_INFO;
    beginResetModel();
    this->qvec = new_qvec;
